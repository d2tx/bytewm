...
path_drm:      .asciz  "/sys/class/drm/card0/device/uevent"

# ── search keys ────────────────────────────────────────
key_pretty:    .asciz  "PRETTY_NAME=\""
key_cpu:       .asciz  "model name\t: "
drm_key:       .asciz  "DRIVER="
...
gather_gpu:
	...
	# copy drm path to writable buffer
	leaq	path_drm(%rip), %rsi
	leaq	drm_path(%rip), %rdi
1:	movb	(%rsi), %al
	movb	%al, (%rdi)
	incq	%rsi
	incq	%rdi
	...
	# Try card1 (dGPU) first, then card0 (iGPU)
	movq	$1, %r12
gg_try:
	cmpq	$0, %r12
	jl	gg_done

	movb	$'0', drm_path+19(%rip)
	addb	%r12b, drm_path+19(%rip)

	leaq	drm_path(%rip), %rsi
	leaq	buf_osrel(%rip), %rdx
	movq	$4096, %rcx
	call	read_file
	cmpq	$0, %rax
	...
	leaq	buf_osrel(%rip), %rsi
	movq	%r13, %rdx
	leaq	drm_key(%rip), %rdi
	call	find_after
	testq	%rax, %rax
	jz	gg_next

	# found — copy driver name to out_gpu
	movq	%rax, %rsi
	leaq	out_gpu(%rip), %rdi
	movb	$'\n', %dl
	movb	$31, %cl
	call	copy_until_strict
	jmp	gg_cap
